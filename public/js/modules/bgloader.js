...
export class BgLoader {
	...
	async loadSlideBackground(slide, inc) {
		if (this.images.length <= 0) {
			return;
		}
	...
	onTransitionEnd() {
		if (this.direction !== 0) {
			let first = this.slider.firstElementChild,
				last = this.slider.lastElementChild,
				updatedSlide,
				loadIndex;
